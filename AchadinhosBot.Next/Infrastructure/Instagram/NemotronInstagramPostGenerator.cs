...
public sealed class NemotronInstagramPostGenerator
{
    private static int _currentKeyIndex = -1;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<NemotronInstagramPostGenerator> _logger;
    ...
    public NemotronInstagramPostGenerator(
        IHttpClientFactory httpClientFactory,
        ILogger<NemotronInstagramPostGenerator> logger,
    ...
    public async Task<string?> GenerateAsync(string productInput, string? offerContext, string? affiliateLink, InstagramPostSettings instaSettings, NemotronSettings aiSettings, CancellationToken cancellationToken)
    {
        ...
        var model = string.IsNullOrWhiteSpace(aiSettings.Model) ? "nvidia/nemotron-3-super-120b-a12b" : aiSettings.Model.Trim();
        var baseUrl = string.IsNullOrWhiteSpace(aiSettings.BaseUrl) ? "https://integrate.api.nvidia.com/v1" : aiSettings.BaseUrl.Trim();
        ...
        var payload = new
        {
            ...
            reasoning_budget = aiSettings.ReasoningBudget,
            chat_template_kwargs = new { enable_thinking = aiSettings.EnableThinking },
            stream = false
        };
        ...
        try
        {
            ...
            await _logStore.AppendAsync(BuildLogEntry(instaSettings, model, "nemotron", productInput, affiliateLink, images.Count, images, finalText, null, started, score, notes), cancellationToken);
            return finalText;
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Falha ao gerar post via Nemotron");
            await _logStore.AppendAsync(BuildLogEntry(instaSettings, model, "nemotron", productInput, affiliateLink, 0, Array.Empty<string>(), null, ex.Message, started, 0, "Erro na geracao"), cancellationToken);
            return null;
        }
    }

    public async Task<string?> GenerateFreeformAsync(string prompt, NemotronSettings aiSettings, CancellationToken cancellationToken)
    {
        var apiKeys = GetApiKeys(aiSettings);
        ...
        {
            _logger.LogWarning(ex, "Falha ao gerar resposta livre via Nemotron");
            return null;
        }
    }
    ...
    {
        var client = _httpClientFactory.CreateClient("nemotron");
        ...
        for (var i = 0; i < apiKeys.Count; i++)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning("Nemotron respondeu erro {Status}: {Body}", response.StatusCode, body);
                return null;
            }
        ...
        _logger.LogWarning("Nemotron falhou em todas as chaves: {Body}", lastBody);
        return null;
    }

    private static List<string> GetApiKeys(NemotronSettings settings)
    {
    ...
    private static (int Score, string Notes) EvaluateQuality(string? text)
    {
        ...
        if (!text.Contains("Legenda 1", StringComparison.OrdinalIgnoreCase)) { score -= 20; notes.Add("sem variacoes"); }
        if (!text.Contains("Hashtags", StringComparison.OrdinalIgnoreCase)) { score -= 15; notes.Add("sem hashtags"); }
        if (text.Contains("lorem", StringComparison.OrdinalIgnoreCase)) { score -= 30; notes.Add("texto artificial"); }
        return (Math.Clamp(score, 0, 100), notes.Count == 0 ? "ok" : string.Join(", ", notes));
    }
}
