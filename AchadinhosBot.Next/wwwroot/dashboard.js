let currentRole = null;
  ...
  `).join('');
    ...
    function renderMercadoLivrePending(items) {
      ...
      if (!items || items.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="muted">Sem pend̻ncias.</td></tr>';
    ...
    async function approveMercadoLivrePending(id) {
      if (currentRole !== 'admin') return;
      const note = prompt('Observa̤̣o de aprova̤̣o (opcional):', '') || '';
      ...
      try {
        ...
        alert(e?.data?.error || e?.message || 'Erro ao aprovar pend̻ncia.');
      }
    }
